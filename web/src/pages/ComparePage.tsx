...
export default function ComparePage() {
  ...
  const queries = useQuery({
    ...
    queryFn: async () => {
      const results = await Promise.all([
        ...
        api.GET('/api/instances/{instance_id}/presence-timeline', { params: { path: { instance_id: id1 } } }),
        api.GET('/api/instances/{instance_id}/presence-timeline', { params: { path: { instance_id: id2 } } }),
        api.GET('/api/instances/{instance_id}/sessions', { params: { path: { instance_id: id1 } } }),
        api.GET('/api/instances/{instance_id}/sessions', { params: { path: { instance_id: id2 } } }),
      ])
  ...
  if (!valid) {
    return <Alert severity="error" className="m-3">URLパラメータ id1, id2 が必要です。</Alert>
  }
  if (queries.error) {
    return <Alert severity="error" className="m-3">データの読み込みに失敗しました</Alert>
  }
  if (!derived) {
    return <Typography color="text.secondary" className="p-3">読み込み中...</Typography>
  }
          ...
          <CardHeader
            ...
            title={
              <Typography variant="caption" color="text.secondary" className="font-semibold">
                人数差分（<Box component="span" color="primary.main">青</Box> −{' '}
                <Box component="span" color="error.main">赤</Box>）
              </Typography>
            }
          />
          <CardContent>
            ...
            title={
              <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                  <Typography variant="caption" component="label">猶予</Typography>
                  <Select
                    size="small"
                    value={grace}
  ...
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      ...
      zoom: {
        ...
        pan: { ...chartZoomOptions.pan, onPan: ({ chart }) => syncZoom(chart, otherChartRef.current) },
      },
    },
  }
  return (
...
function ViolationsTable({ violations, sort, onSort, highlightedTs, onPickTime, id1, id2 }: ViolationsTableProps) {
  const sorted = useMemo(() => {
    return [...violations].sort((a, b) => {
      ...
      if (sort.by === 'join_ts') {
        va = a.join_ts.getTime(); vb = b.join_ts.getTime()
      } else if (sort.by === 'duration_seconds') {
        va = a.duration_seconds ?? -1; vb = b.duration_seconds ?? -1
      } else {
        va = a[sort.by] as string; vb = b[sort.by] as string
      }
              ...
              >
                ...
                <TableCell>
                  <Chip
                    ...
                    sx={{ backgroundColor: color === 'blue' ? '#0d6efd' : '#dc3545', color: '#fff' }}
                  />
                </TableCell>
                <TableCell>+{v.diff}</TableCell>
                <TableCell>{v.duration_seconds != null ? fmtDuration(v.duration_seconds) : '—'}</TableCell>
              </TableRow>
            )
          })}
        </TableBody>
